...
#include <cstring>
...

  {
    ...
    tt->help = tdrpStrDup("Increase the value if processing takes a long time. This will prevent the auto_restarter from killing and restarting the app.");
    ...
    tt->descr = tdrpStrDup("Seconds to sleep after triggering on an input file");
    tt->help = tdrpStrDup("REALTIME mode only.");
    ...
    tt->descr = tdrpStrDup("URL of the input field");
    tt->help = tdrpStrDup("If set to \"\", will use TriggerUrl");
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("ThreshUrl");
    tt->descr = tdrpStrDup("URL of the field used for thresholding the data");
    tt->help = tdrpStrDup("If set to \"\", will use TriggerUrl");
    tt->val_offset = (char *) &ThreshUrl - &_start_;
    tt->single_val.s = tdrpStrDup("");
    tt++;
    
    // Parameter 'ThreshFieldName'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("ThreshFieldName");
    tt->descr = tdrpStrDup("Name of threshold field to use from the input file - or use '#2' to indicate field two (fields start at 0 in this case).");
    tt->help = tdrpStrDup("If set to \"\", will use each input field as its own threshold field.");
    tt->val_offset = (char *) &ThreshFieldName - &_start_;
    tt->single_val.s = tdrpStrDup("");
    tt++;
    
    // Parameter 'ThreshValidSecs'
    // ctype is 'long'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = LONG_TYPE;
    tt->param_name = tdrpStrDup("ThreshValidSecs");
    tt->descr = tdrpStrDup("Number of seconds the ThreshUrl is valid.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &ThreshValidSecs - &_start_;
    tt->single_val.l = 600;
    tt++;
    
    // Parameter 'OneValueMin'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("OneValueMin");
    tt->descr = tdrpStrDup("Use single value minimum thresh or use grid");
    tt->help = tdrpStrDup("True to use MinThresh.  If false, set minThreshField to a 2D field");
    tt->val_offset = (char *) &OneValueMin - &_start_;
    ...
    tt->param_name = tdrpStrDup("MinThresh");
    tt->descr = tdrpStrDup("Minimum threshold - data must be equal to or\ngreater than this to pass.");
    tt->help = tdrpStrDup("Set to hugely negative value to have no effect.");
    tt->val_offset = (char *) &MinThresh - &_start_;
    tt->single_val.d = -5000;
    tt++;
    
    // Parameter 'MinThreshUrl'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("MinThreshUrl");
    tt->descr = tdrpStrDup("URL of the 2D field used to set minimum threshold");
    tt->help = tdrpStrDup("Must be a 2D field on the same grid as the threshold field");
    tt->val_offset = (char *) &MinThreshUrl - &_start_;
    tt->single_val.s = tdrpStrDup("");
    tt++;
    
    // Parameter 'MinThreshFieldName'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("MinThreshFieldName");
    tt->descr = tdrpStrDup("Name of minimum threshold field to use from the input file - or use '#2' to indicate field two (fields start at 0 in this case).");
    tt->help = tdrpStrDup("Must be a 2D field on the same grid as the threshold field");
    tt->val_offset = (char *) &MinThreshFieldName - &_start_;
    tt->single_val.s = tdrpStrDup("");
    tt++;
    
    // Parameter 'OneValueMax'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("OneValueMax");
    tt->descr = tdrpStrDup("Use single value maximum thresh or use grid");
    tt->help = tdrpStrDup("True to use MaxThresh.  If false, set maxThreshField to a 2D field");
    tt->val_offset = (char *) &OneValueMax - &_start_;
    tt->single_val.b = pTRUE;
    tt++;
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("MaxThresh");
    tt->descr = tdrpStrDup("Maximum threshold - data must be equal to or\nless than this to pass.");
    tt->help = tdrpStrDup("Set to hugely positive value to have no effect.");
    tt->val_offset = (char *) &MaxThresh - &_start_;
    tt->single_val.d = 5000;
    tt++;
    
    // Parameter 'MaxThreshUrl'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("MaxThreshUrl");
    tt->descr = tdrpStrDup("URL of the 2D field used to set maximum threshold");
    tt->help = tdrpStrDup("Must be a 2D field on the same grid as the threshold field");
    tt->val_offset = (char *) &MaxThreshUrl - &_start_;
    tt->single_val.s = tdrpStrDup("");
    tt++;
    
    // Parameter 'MaxThreshFieldName'
    // ctype is 'char*'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = STRING_TYPE;
    tt->param_name = tdrpStrDup("MaxThreshFieldName");
    tt->descr = tdrpStrDup("Name of maximum threshold field to use from the input file - or use '#2' to indicate field two (fields start at 0 in this case).");
    tt->help = tdrpStrDup("Must be a 2D field on the same grid as the threshold field");
    tt->val_offset = (char *) &MaxThreshFieldName - &_start_;
    tt->single_val.s = tdrpStrDup("");
    tt++;
    
    // Parameter 'ThreshMissing'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("ThreshMissing");
    tt->descr = tdrpStrDup("Flag indicating whether to threshold out grid points where data is missing in the threshhold field.");
    tt->help = tdrpStrDup("");
    tt->val_offset = (char *) &ThreshMissing - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    
    // Parameter 'ReplacePassValues'
    // ctype is 'tdrp_bool_t'
    
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = BOOL_TYPE;
    tt->param_name = tdrpStrDup("ReplacePassValues");
    tt->descr = tdrpStrDup("Set this to replace values that pass the thresholding\nwith a contsant value specified by PassValue");
    tt->help = tdrpStrDup("If FALSE the data values that pass are sent to the output.");
    tt->val_offset = (char *) &ReplacePassValues - &_start_;
    tt->single_val.b = pFALSE;
    tt++;
    ...
    memset(tt, 0, sizeof(TDRPtable));
    tt->ptype = DOUBLE_TYPE;
    tt->param_name = tdrpStrDup("PassValue");
    tt->descr = tdrpStrDup("Value to use for data points that pass the threshold test.");
    ...
    tt->descr = tdrpStrDup("Output units for each field.");
    tt->help = tdrpStrDup("Must be output units for each input field.");
    ...
    tt->array_vals = (tdrpVal_t *)
        tdrpMalloc(tt->array_n * sizeof(tdrpVal_t));
      tt->array_vals[0].s = tdrpStrDup("albedo");
      tt->array_vals[1].s = tdrpStrDup("DegC");
    tt++;
