...
# ifndef    ThresholdsAtGenHms_hh
# define    ThresholdsAtGenHms_hh
...
public:
   ...
   */
  ThresholdsAtGenHms(const std::string &xml,
   ...
   * Destructor
   */
  virtual ~ThresholdsAtGenHms(void);
  ...
  */
   ...
   */
  bool replaceValues(const ThresholdsAtGenHms &filtMap,
  ...
  void print(const std::vector<int> &genHours,
	     const std::vector<int> &leadSeconds, 
	     const std::vector<int> &tiles, const TileInfo &info,
...
private:  
   ...
   */
  std::map<int, MultiTileThresholds> _map;

  /**
   * @return pointer to the MultiTileThresholds that goes with the leadtime if
   * there is one, or NULL if no such leadtime in the map
   * @param[in] leadTime
   */
  const MultiTileThresholds *_mapFromLeadTime(int leadTime) const;
