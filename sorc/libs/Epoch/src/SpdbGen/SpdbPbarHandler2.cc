...
//------------------------------------------------------------------------
SpdbPbarHandler2::
SpdbPbarHandler2(const std::string &spdb) :
...
SpdbPbarHandler2::
SpdbPbarHandler2(const std::string &spdb,
		const SpdbPbarMetadata2 &md) :
  SpdbPbarMetadata2(md), _url(spdb), _chunkValidTime(0), _chunkTimeWritten(0)
{
}

//------------------------------------------------------------------------
SpdbPbarHandler2::~SpdbPbarHandler2(void)
{
}

//------------------------------------------------------------------------
bool SpdbPbarHandler2::read(const time_t &genTime)
{
  return _readExisting(genTime, "Exact");
}
		
//------------------------------------------------------------------------
bool SpdbPbarHandler2::readBestOlder(const time_t &genTime,
				    int maxSecondsBack)
{
...
//------------------------------------------------------------------------
bool SpdbPbarHandler2::readNewestInRange(const time_t &t0,
					const time_t &t1)
{
...
//------------------------------------------------------------------------
bool SpdbPbarHandler2::readClosestToTargetInRange(const time_t &target,
						 const time_t &t0,
						 const time_t &t1)
{
  std::vector<time_t>  times = timesInRange(t0, t1);
...
//------------------------------------------------------------------
bool SpdbPbarHandler2::write(void)
{
  return write(_url);
}

//------------------------------------------------------------------
bool SpdbPbarHandler2::write(const std::string &url)
{
  string xml = SpdbPbarMetadata2::toXml();
...
//------------------------------------------------------------------
std::vector<time_t> 
SpdbPbarHandler2::timesInRange(const time_t &t0,
...
//------------------------------------------------------------------
std::vector<time_t> 
SpdbPbarHandler2::timesInRangeWithMatchingHour(const time_t &t0,
					      const time_t &t1) const
{
  DsSpdb D;
  std::vector<time_t> ret;
...
//------------------------------------------------------------------------
bool SpdbPbarHandler2::_readExisting(const time_t &genTime,
...
//------------------------------------------------------------------
bool SpdbPbarHandler2::_load(DsSpdb &s, const std::string &description)
{
  ...
  for (int jj = 0; jj < nChunks; jj++)
  {
    ...
    if (s.getProdId() == SPDB_XML_ID)
    {
      ...
      return SpdbPbarMetadata2::fromXml(xml);
    }
    else
    { 
