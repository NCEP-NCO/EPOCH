...
def get_archive_time(run_time):
  'creates a unix time from from time string in the form YYYYMMDDHHMM'
  ...
  # handle if minutes are not provided
  if len(run_time) < 12:
      minutes = 0
  else:
...
def check_for_file(the_dir):
  ...
  file_path = ldata.rel_data_path(the_dir)
...
#######################################################################
# get_gen_g_f_path(run_time, base_dir, fn_tmplate, fcast='', add_date=False)
#   
# gets the pathname based on generation time  
#

def get_gen_g_f_path(run_time, base_dir, fn_tmplate, fcast=0, add_date=False):

  init_time = datetime.utcfromtimestamp(run_time)

  pathname = base_dir

  date_dir = init_time.strftime('%Y%m%d')

  # get forecast precision from template and replace with fcast value in seconds
  fcast_seconds = str(fcast * 3600)
  match = re.match(r'.*%(\d)F.*', fn_tmplate)
  if match:
      precision = match.group(1)
      fn_tmplate = fn_tmplate.replace('%'+precision+'F',
                                      fcast_seconds.zfill(int(precision)))

  if add_date:
    pathname = path.join(pathname, date_dir, init_time.strftime(fn_tmplate))
  else:
    pathname = path.join(pathname, init_time.strftime(fn_tmplate))
...
if __name__ == "__main__":
  ...
  icing_message.debug_mode = True
  
  run_time = get_archive_time('200406221200')
